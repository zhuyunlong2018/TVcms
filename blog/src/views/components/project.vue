    ...
    <div class="project">
		<div class="project-container clearfix">
			...
			<div class="main-title">
				<h2>实验室</h2>
					<ul>
						<li v-if="item.pr_published==1?true:false" @click="_change_production(index)" v-for="(item,index) in production" :key="item.pr_id" ><a :class="{'selected':show_item==index}" >{{item.pr_name}}</a></li>
					</ul>
			</div>
			<div class="project-container-main">
					<transition-group name="change-page">
							<div v-show="show_page == _index" v-for="(text,_index) in show_production.content" :key="show_production.id" class="project-container-text">
								<h2>
